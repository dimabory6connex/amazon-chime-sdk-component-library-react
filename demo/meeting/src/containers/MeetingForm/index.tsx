...
const MeetingForm: React.FC = () => {
    ...
    const [newRegion, setNewRegion] = useState(appRegion);
    ...
    const handleCreateNewMeeting = useCallback(async (e: React.FormEvent) => {
        e.preventDefault();
        ...
        setIsLoading(true);
        try {
            await fetchMeeting(id, '', newRegion);
            await getMeetings()
        } catch (error) {
    ...
    }, [newMeetingId, newRegion, setNewMeetingErr]);
    ...
    return (
        <>
            <form>
                ...
                <RegionSelection setRegion={setNewRegion} region={newRegion}/>
                ...
                >
                    {isLoading ? (
                        ...
                        <PrimaryButton label="Create new" onClick={handleCreateNewMeeting}/>
                    )}
                </Flex>
            </form>
            ...
            <ul>
                {meetings.map((item: any, index: any) => {
                    ...
                    return <li key={`${name}-${index}`}>
                        <Flex layout="equal-columns" justifyContent="space-around" alignItems="center" css="margin: 1rem">
                            <div>
                                <span>{name}</span>
                            </div>
                            <div>
                                <SecondaryButton label="Join to this meeting" onClick={(e) => {
                                    e.preventDefault();
